...
public: 
        ...
        if(msg != NULL){ 
            if(counter_footprint_ != 0){ 
                if(counter_ == 0){ 
                    ...
                    setThreshold(); 
                    counter_ ++; 
                    ROS_INFO("[%s] (%s) Threshold : %f", ros::this_node::getName().c_str(), frame_.c_str(), threshold_); 
                } 
                range_msg_ = *msg; 
                publish(); 
            } 
    ...
    void setSonarTF(){ 
        ...
        while(ros::ok()){ 
            try{ 
                tf_sonar_ = tfBuffer_.lookupTransform(footprint_.header.frame_id, frame_, ros::Time::now(), ros::Duration(3.0)); 
            } 
    ...
    void setThreshold(){ 
        ...
        for (int i=0 ;i < size ; i++){ 
            ...
            threshold[i]=(sqrt(pow((intersection.x - sonar_initial_point.x), 2) + pow((intersection.y - sonar_initial_point.y), 2)));  
